...
std::string generate_mt_metadata_source(const SystemDecl& system)
{
    ...
    for (const auto& entity : system.entities)
    {
        ...
        out << "}};\n\n";
        out << "constexpr std::array<std::string_view, " << entity.key_fields.size() << "> " << symbol
            << "_keys{{\n";
        for (const auto& key : entity.key_fields)
        {
            out << "    \"" << key << "\",\n";
    ...
    {
        const auto symbol = to_lower(entity.name);
        out << "    EntityMetadata{\"" << entity.name << "\", " << symbol << "_fields.data(), " << symbol
            << "_fields.size(), " << symbol << "_keys.data(), " << symbol << "_keys.size()},\n";
    }
    out << "}};\n\n";
    out << "} // namespace\n\n";
...
std::string generate_dl_metadata_source(const SystemDecl& system)
{
    ...
    out << "}};\n\n";
    out << "constexpr std::array<WorkerLeaseBinding, " << system.workers.size() << "> all_worker_lease_bindings{{\n";
    for (const auto& worker : system.workers)
    {
        out << "    WorkerLeaseBinding{\"" << worker.name << "\", \"" << optional_or_empty(worker.lease)
            << "\", " << bool_text(worker.singleton.value_or(false)) << ", "
            << worker.concurrency.value_or(0) << "},\n";
    }
    out << "}};\n\n";
    out << "} // namespace\n\n";
...
std::string generate_qu_messages_header(const SystemDecl& system)
{
    ...
    out << "const MessageMetadata* messages();\n";
    out << "std::size_t message_count();\n";
    out << "std::string_view idempotency_key_field(std::string_view queue_name, std::string_view message_name);\n\n";
    out << "} // namespace statespec_generated::qu\n";
    return out.str();
}
...
std::string generate_qu_metadata_source(const SystemDecl& system)
{
    ...
    for (const auto& queue : system.queues)
    {
        for (const auto& message : queue.messages)
        {
            ...
            out << "    MessageMetadata{\"" << queue.name << "\", \"" << message.name << "\", \""
                << payload_struct_name(queue, message) << "\", \"" << optional_or_empty(message.idempotency_key)
                << "\", " << symbol << "_payload_fields.data(), " << symbol
                << "_payload_fields.size()},\n";
        }
    }
    out << "}};\n\n";
    ...
    for (const auto& queue : system.queues)
    {
        ...
        for (const auto& message : queue.messages)
        {
            ...
            out << "    MessageMetadata{\"" << queue.name << "\", \"" << message.name << "\", \""
                << payload_struct_name(queue, message) << "\", \"" << optional_or_empty(message.idempotency_key)
                << "\", " << message_symbol << "_payload_fields.data(), " << message_symbol
                << "_payload_fields.size()},\n";
        }
        out << "}};\n\n";
    }
    ...
    {
        const auto symbol = to_lower(queue.name);
        out << "    QueueMetadata{\"" << queue.name << "\", \"" << optional_or_empty(queue.namespace_name)
            << "\", \"" << optional_or_empty(queue.channel) << "\", \""
            << optional_or_empty(queue.visibility_timeout) << "\", " << queue.max_attempts.value_or(0)
            << ", " << symbol << "_messages.data(), " << symbol << "_messages.size()},\n";
    }
    out << "}};\n\n";
    out << "} // namespace\n\n";
