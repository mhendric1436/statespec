...
std::string generate_rust_runtime_registration(const IrSystem&)
{
    ...
    out << "    let result = register_runtime_catalog_tx(\n";
    out << "        &mut tx,\n";
    ...
    out << "        metric_sink,\n";
    out << "    );\n";
    out << "    if let Err(error) = result {\n";
    out << "        let _ = tx.abort();\n";
    out << "        return Err(error);\n";
    out << "    }\n";
