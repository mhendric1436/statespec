...
std::string generate_system_descriptors_header(const Spec& spec)
{
    ...
    if (spec.system.has_value())
    {
        for (const auto& queue : spec.system->queues)
        {
            ...
            out << "            std::chrono::seconds{" << parse_duration_seconds(queue.visibility_timeout) << "},\n";
    ...
    if (spec.system.has_value())
    {
        for (const auto& lease : spec.system->leases)
        {
            ...
            {
                out << "            std::optional<std::chrono::seconds>{std::chrono::seconds{" << parse_duration_seconds(lease.renew_every) << "}},\n";
            }
            else
            {
                out << "            std::nullopt,\n";
            ...
            {
                out << "            std::optional<std::chrono::seconds>{std::chrono::seconds{" << parse_duration_seconds(lease.max_ttl) << "}},\n";
            }
            else
            {
                out << "            std::nullopt,\n";
    ...
    if (spec.system.has_value())
    {
        for (const auto& workflow : spec.system->workflows)
        {
            ...
            out << "            std::chrono::seconds{" << parse_duration_seconds(workflow.expected_execution_time) << "},\n";
            out << "            " << (workflow.singleton.value_or(false) ? "true" : "false") << ",\n";
            out << "            {\n";
            for (const auto& step : workflow.steps)
            {
                out << "                statespec::backend::WorkflowStepDefinition{"
                    << cpp_string(step.name) << ", std::chrono::seconds{" << parse_duration_seconds(step.expected_execution_time)
