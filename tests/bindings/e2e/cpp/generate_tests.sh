...
assert_file_contains "$TMPDIR/out-app-cpp/worker/workflows/provision_service/registry.hpp" "return handler.handle_validate_request(context);"
...
assert_file_contains "$TMPDIR/out-app-cpp/worker/workflow_runner.hpp" "const auto result = invoker->second(backend_, context)"
