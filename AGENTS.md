...
completing, failing, or canceling a claimed step. C++, Go, and Java generated workflow runners
start a periodic keep-alive controller while the handler transaction is open; Rust
still uses the single pre-handler keep-alive baseline until its periodic
